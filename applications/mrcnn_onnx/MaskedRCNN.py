...
from keras2onnx._builtin import on_StridedSlice, on_Round, on_TopKV2, on_Pad, on_CropAndResize
...
def convert_DetectionLayer(scope, operator, container):
    ...
    apply_transpose(scope, score_identity, score_transpose, container, perm=[0, 2, 1])
    ...
    container.add_node("Slice",
                       [nms_output, starts_init, ends_init, axes_init],
                       class_idx_output,
                       op_version=operator.target_opset,
    ...
    container.add_node("Slice",
                       [nms_output, starts_init_2, ends_init_2, axes_init_2],
                       box_idx_output,
                       op_version=operator.target_opset,
    ...
    container.add_node("Slice",
                       [nms_output, starts_init_3, ends_init_3, axes_init_3, step_init_3],
...
if len(sys.argv) > 1 and sys.argv[1] == '-c':
    ...
    for n_ in range(len(results)):
        print(str(n_))
        expected_list = results[n_].flatten()
        actual_list = results_k[n_].flatten()
