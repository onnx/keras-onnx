...
from ..common.onnx_ops import apply_softmax
...
def activation_process(scope, operator, container, biased_tensor_name):
    ...
    if operator.raw_operator.activation in [activation_get('softmax'), keras.activations.softmax]:
        ...
        apply_activation_function(scope, biased_tensor_name, operator.outputs[0].full_name, container, dtype=operator.raw_operator.input.dtype.as_numpy_dtype)
    else:
