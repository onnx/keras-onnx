...
def convert_keras_conv_core(scope, operator, container, is_transpose, n_dims, input_perm_axes,
    ...
    input_shape = op._input_shape if hasattr(op, '_input_shape') else operator.get_input_shape()
    output_shape = op._output_shape if hasattr(op, '_output_shape') else operator.get_output_shape()
    padded_result = None
